...
Use the `aws_transit_gateway_routes` InSpec audit resource to test the properties of all Route for a Amazon Transit Gateway Route Table.
...
Additional params `exact_match`, `longest_prefix_match`, `subnet_of_match`, `supernet_of_match` are optional.
...
#### supernet\_of\_match _(optional)_
...
|types                     | The type of all routes (`propagated` | `static`).          | type           |
|states                    | The state of the routes (`active` | `blackhole`).          | state          |
...
### Filter all static transit gateway routes that are blackhole
