...
class AWSWAFV2XSSMatchSet < AwsResourceBase
  name "aws_wafv2_xss_match_set"
  desc "Describes one WAF XSS match set."

  example "
    describe aws_wafv2_xss_match_set(xss_match_set_id: 'XSS_MATCH_SET_ID') do
      it { should exits }
    end
  "
  ...
  def initialize(opts = {})
    ...
    catch_aws_errors do
      resp = @aws.waf_client_v2.get_xss_match_set({ xss_match_set_id: opts[:xss_match_set_id] })
