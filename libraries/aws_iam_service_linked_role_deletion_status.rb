...
class AWSIAMServiceLinkedRoleDeletionStatus < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:deletion_task_id] && !opts[:deletion_task_id].empty?
      raise ArgumentError,
            "#{@__resource_name__}: deletion_task_id must be provided"
    end
    @display_name = opts[:deletion_task_id]
    catch_aws_errors do
      resp =
        @aws.iam_client.get_service_linked_role_deletion_status(
          { deletion_task_id: opts[:deletion_task_id] }
        )
      @res = resp.to_h
