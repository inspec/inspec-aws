...
class AWSIAMServiceLinkedRoleDeletionStatus < AwsResourceBase
  ...
  example "
    describe aws_iam_service_linked_role_deletion_status(deletion_task_id: 'IAM_SERVICE_LINKED_DELETION_TASK_ID') do
      it { should exist }
    end
  "
  ...
  def to_s
    "IAM Service Linked Deletion Task ID: #{@display_name}"
  end
end
