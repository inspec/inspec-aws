...
class AwsIamAccessKeys < AwsCollectionResourceBase
  ...
  FilterTable
    ...
    .register_column(
      ...
      lazy_instance: :lazy_load_last_used_date,
    )
    .register_column(
      :last_used_hours_ago,
      field: :last_used_hours_ago,
      lazy_instance: :lazy_load_last_used_hours_ago,
    )
    .register_column(
      :last_used_days_ago,
      field: :last_used_days_ago,
      lazy_instance: :lazy_load_last_used_days_ago,
    )
    .register_column(
      :ever_used,
      field: :ever_used,
      lazy_instance: :lazy_load_ever_used,
    )
    .register_column(
      :never_used,
      field: :never_used,
      lazy_instance: :lazy_load_never_used_time,
    )
  ...
  def get_users(username = nil)
    catch_aws_errors { username ? [fetch_user(username)] : collect_all_users }
  end
  ...
  def collect_all_users
    catch_aws_errors { iam_client.list_users.flat_map(&:users) }
  end

  # Given a Hash of Users, build Access Key details for each.
  def get_keys
    @_users.flat_map { |user| fetch_keys(user.user_name) }
  end
  ...
  def fetch_keys(username)
    ...
    access_keys&.flat_map do |response|
      response.access_key_metadata.flat_map do |access_key|
        ...
        access_key_hash[:created_hours_ago] = (
          (Time.now - access_key_hash[:create_date]) / (60 * 60)
        ).to_i
        access_key_hash[:created_days_ago] = (
          access_key_hash[:created_hours_ago] / 24
        ).to_i
        access_key_hash[:user_created_date] = @_users
          .find { |user| user.user_name == access_key_hash[:username] }
          .create_date
        access_key_hash[:created_with_user] = (
          access_key_hash[:create_date] - access_key_hash[:user_created_date]
        ).abs < 1.0 / 24.0
        access_key_hash
      end
    end
  ...
  def lazy_load_last_used_hours_ago(row, condition, table)
    return row[:last_used_hours_ago] = nil if lazy_load_never_used_time(
      row,
      condition,
      table,
    )
    row[:last_used_hours_ago] = (
      (Time.now - row[:last_used_date]) / (60 * 60)
    ).to_i
  end

  def lazy_load_last_used_days_ago(row, condition, table)
    return row[:last_used_days_ago] = nil if lazy_load_never_used_time(
      row,
      condition,
      table,
    )
