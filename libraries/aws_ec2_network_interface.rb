...
class AWSEC2NetworkInterface < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:network_interface_id] && !opts[:network_interface_id].empty?
      raise ArgumentError,
            "#{@__resource_name__}: network_interface_id must be provided"
    end
    @display_name = opts[:network_interface_id]
    catch_aws_errors do
      resp =
        @aws.compute_client.describe_network_interfaces(
          { network_interface_ids: [opts[:network_interface_id]] }
        )
