...
class AWSBatchJobQueue < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:job_queue_name] && !opts[:job_queue_name].empty?
      raise ArgumentError,
            "#{@__resource_name__}: job_queue_name must be provided"
    end
    @display_name = opts[:job_queue_name]
    catch_aws_errors do
      resp =
        @aws.batch_client.describe_job_queues(
          { job_queues: [opts[:job_queue_name]] }
        )
