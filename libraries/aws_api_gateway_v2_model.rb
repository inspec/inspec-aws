...
class AwsApiGatewayV2Model < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    if opts[:model_id].blank?
      raise ArgumentError, "#{@__resource_name__}: model_id must be provided!"
    end
    @display_name = opts[:model_id]
    catch_aws_errors do
      resp =
        @aws.apigatewayv2_client.get_model(
          { api_id: opts[:api_id], model_id: opts[:model_id] }
        )
      @res = resp.to_h
