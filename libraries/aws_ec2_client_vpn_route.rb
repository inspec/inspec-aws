...
class AWSEC2ClientVPNRoute < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:target_subnet] && !opts[:target_subnet].empty?
      raise ArgumentError,
            "#{@__resource_name__}: target_subnet must be provided"
    end
    @display_name = opts[:client_vpn_endpoint_id]
    filter = [{ name: "target-subnet", values: [opts[:target_subnet]] }]
    catch_aws_errors do
      resp =
        @aws.compute_client.describe_client_vpn_routes(
          {
