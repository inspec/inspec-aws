...
class AWSStepFunctionsStateMachine < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:state_machine_arn] && !opts[:state_machine_arn].empty?
      raise ArgumentError,
            "#{@__resource_name__}: state_machine_arn must be provided"
    end
    @display_name = opts[:state_machine_arn]
    catch_aws_errors do
      resp =
        @aws.states_client.describe_state_machine(
          { state_machine_arn: opts[:state_machine_arn] }
        )
      @res = resp.to_h
