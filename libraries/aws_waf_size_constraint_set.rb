...
class AWSWAFSizeConstraintSet < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:size_constraint_set_id] &&
             !opts[:size_constraint_set_id].empty?
      raise ArgumentError,
            "#{@__resource_name__}: size_constraint_set_id must be provided"
    end
    @display_name = opts[:size_constraint_set_id]
    catch_aws_errors do
      resp =
        @aws.waf_client.get_size_constraint_set(
          { size_constraint_set_id: opts[:size_constraint_set_id] }
        )
