...
class AWSWAFV2SQLInjectionMatchSet < AwsResourceBase
  name "aws_wafv2_sql_injection_match_set"
  desc "Describes one WAF SQL Injection match set."

  example "
    describe aws_wafv2_sql_injection_match_set(sql_injection_match_set_id: 'SQL_INJECTION_MATCH_SET_ID') do
      it { should exits }
    end
  "
  ...
  def initialize(opts = {})
    ...
    catch_aws_errors do
      resp = @aws.waf_client_v2.get_sql_injection_match_set({ sql_injection_match_set_id: opts[:sql_injection_match_set_id] })
