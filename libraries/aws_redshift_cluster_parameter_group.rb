...
class AwsRedshiftClusterParameterGroup < AwsResourceBase
  ...
  def initialize(opts = {})
    ...
    unless opts[:parameter_group_name] && !opts[:parameter_group_name].empty?
      raise ArgumentError,
            "#{@__resource_name__}: parameter_group_name must be provided"
    end
    @display_name = opts[:parameter_group_name]
    catch_aws_errors do
      resp =
        @aws.redshift_client.describe_cluster_parameter_groups(
          { parameter_group_name: opts[:parameter_group_name] }
        )
