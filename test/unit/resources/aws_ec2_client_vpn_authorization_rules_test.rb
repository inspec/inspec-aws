...
class AWSEC2ClientVPNRulesConstructorTest < Minitest::Test
  def test_empty_params_not_ok
    assert_raises(ArgumentError) { AWSEC2ClientVPNRules.new(client_args: { stub_responses: true }) }
  end
  ...
  def test_rejects_unrecognized_params
    assert_raises(ArgumentError) { AWSEC2ClientVPNRules.new(rubbish: 9) }
  end
end

class AWSEC2ClientVPNRulessPathTest < Minitest::Test
  def setup
    data = {}
    ...
    @resp = AWSEC2ClientVPNRules.new(client_vpn_endpoint_id: 'test', client_args: { stub_responses: true }, stub_data: [data])
  end
