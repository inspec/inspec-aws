require 'helper'
require 'aws_ec2_client_vpn_target_network_associations'
require 'aws-sdk-core'

class AWSEC2ClientVPNTargetNetworkAssociationsConstructorTest < Minitest::Test
  def test_empty_params_not_ok
    assert_raises(ArgumentError) { AWSEC2ClientVPNTargetNetworkAssociations.new(client_args: { stub_responses: true }) }
  end

  def test_accepts_client_vpn_endpoint_id_as_hash_eight_sign
    AWSEC2ClientVPNTargetNetworkAssociations.new(client_vpn_endpoint_id: 'test', client_args: { stub_responses: true })
  end
  ...
  def test_rejects_unrecognized_params
    assert_raises(ArgumentError) { AWSEC2ClientVPNTargetNetworkAssociations.new(rubbish: 9) }
  end
end

class AWSEC2ClientVPNTargetNetworkAssociationssPathTest < Minitest::Test
  def setup
    data = {}
