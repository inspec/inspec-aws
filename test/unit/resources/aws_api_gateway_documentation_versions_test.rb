...
class AWSApiGatewayDocumentationVersionsHappyPathTest < Minitest::Test
  ...
  def setup
    ...
    @items = AWSApiGatewayDocumentationVersions.new(rest_api_id: 'test1', client_args: { stub_responses: true }, stub_data: [data])
  end
  ...
  def test_versions
    assert_equal(@items.versions, ['test'])
  end

  def test_descriptions
    assert_equal(@items.descriptions, ['test'])
  end
