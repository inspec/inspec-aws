...
class AWSElasticLoadBalancingV2LoadBalancerConstructorTest < Minitest::Test
  ...
  def test_empty_param_arg_not_ok
    assert_raises(ArgumentError) { AWSElasticLoadBalancingV2LoadBalancer.new(load_balancer_arns: '', client_args: { stub_responses: true }) }
  end
...
class AWSElasticLoadBalancingV2LoadBalancerSuccessPathTest < Minitest::Test
  ...
  def setup
    ...
    @load_balancers = AWSElasticLoadBalancingV2LoadBalancer.new(load_balancer_arns: 'test1', client_args: { stub_responses: true }, stub_data: [data])
  end
