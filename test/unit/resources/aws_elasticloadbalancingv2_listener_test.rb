...
class AWSElasticLoadBalancingV2ListenerConstructorTest < Minitest::Test
  ...
  def test_empty_param_arg_not_ok
    assert_raises(ArgumentError) { AWSElasticLoadBalancingV2Listener.new(listener_arns: '', client_args: { stub_responses: true }) }
  end
...
class AWSElasticLoadBalancingV2ListenerSuccessPathTest < Minitest::Test
  ...
  def setup
    ...
    mock_parameter[:port] = 'test1'
    data[:data] = { listeners: mock_parameter }
    data[:client] = Aws::ElasticLoadBalancingV2::Client
    @listeners = AWSElasticLoadBalancingV2Listener.new(listener_arns: 'test1', client_args: { stub_responses: true }, stub_data: [data])
  end
  ...
  def test_listener_arn
    assert_equal(@rules.listener_arn, 'test1')
  end

  def test_load_balancer_arn
    assert_equal(@rules.load_balancer_arn, 'test1')
  end

  def test_port
    assert_equal(@rules.port, 'test1')
  end
end