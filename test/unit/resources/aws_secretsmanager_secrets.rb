...
class AWSSecretsManagerSecretsConstructorTest < Minitest::Test
    ...
    def test_empty_params_not_ok
        assert_raises(ArgumentError) { AWSSecretsManagerSecrets.new(client_args: { stub_responses: true }) }
    end
...
class AWSSecretsManagerSecretsSuccessPathTest < Minitest::Test
    ...
    def test_last_rotated_dates
        assert_equal(@secret_list.last_rotated_dates, Time.parse(1523482025.685))
    end

    def test_last_changed_dates
        assert_equal(@secret_list.last_changed_dates, Time.parse(1523482025.685))
    end

    def test_last_accessed_dates
        assert_equal(@secret_list.last_accessed_dates, Time.parse(1523482025.685))
    end

    def test_deleted_dates
        assert_equal(@secret_list.deleted_dates, Time.parse(1523482025.685))
    end
    ...
    def test_secret_versions_to_stages
        assert_equal(@secret_list.secret_versions_to_stages, {})
    end
    ...
    def test_created_dates
        assert_equal(@secret_list.created_dates, Time.parse(1523482025.685))
    end
