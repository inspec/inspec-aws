...
control 'aws-elasti-cache-cluster-node-1.0' do

  impact 1.0
  title 'Ensure AWS ElastiCache Cluster node has the correct properties.'

  aws_elasti_cache_cluster(cache_cluster_id: aws_elasticache_cluster_id).node_ids.each do |node_id|
    describe aws_elasti_cache_cluster_node(cache_cluster_id: aws_elasticache_cluster_id, node_id: node_id) do
      it { should exist }
