...
aws_vpce_name = attribute(:aws_vpce_id, default: '', description: 'The AWS VPC Endpoint Name.')
aws_vpc_id = attribute(:aws_vpce_type, default: '', description: 'The VPC ID where the endpoint resides')
...
control 'aws-vpcendpoints-1.0' do
  ...
  describe aws_vpc_endpoints do
    ...
    its('vpc_endpoint_types')           { should be_in ["Interface", "Gateway"] }
    its('vpc_ids')            { should include aws_vpc_id }
    its('service_names')  { should include aws_vpce_service_name }
    its('states') { should be_in ["PendingAcceptance", "Pending", "Available", "Deleting", "Deleted", "Rejected", "Failed", "Expired"] }
    its('route_table_ids')       { should be_in [true, false] }
