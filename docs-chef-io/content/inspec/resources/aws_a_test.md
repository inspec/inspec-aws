...
[menu.inspec]
title = "aws_a_test"
identifier = "inspec/resources/aws/aws_a_test Resource"
parent = "inspec/resources/aws"
+++

Use the `aws_a_test` InSpec audit resource to test properties of a single AWS Application Load Balancer (ALB).
...
Ensure that an `aws_a_test` exists

```ruby
describe aws_a_test('arn:aws:elasticloadbalancing') do
  it { should exist }
end
```

```ruby
describe aws_a_test(load_balancer_arn: 'arn:aws:elasticloadbalancing') do
  it { should exist }
end
```
...
```ruby
describe aws_a_test('arn::alb') do
...
```ruby
describe aws_a_test('load_balancer_arn::alb') do
  it                         { should exist }
...
```ruby
describe aws_a_test('AnExistingALB') do
  it { should exist }
end
```

```ruby
describe aws_a_test('ANonExistentALB') do
  it { should_not exist }
end
```
